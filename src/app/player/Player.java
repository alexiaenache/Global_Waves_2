...
public class Player {
    ...
    public void logOut() {
        if( source != null && source.getType().equals(Enums.PlayerSourceType.SONG)) {
            lastAudionBeforeLogOut =  new PlayerSource(source.getType(), source.getAudioFile(), new SongBookmark(
                source.getAudioFile().getName(), source.getRemainedDuration(), paused, repeatMode
            ));
        }
//        if(source != null) {
//            source.resetPlayer();
